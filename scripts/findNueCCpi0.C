...
void
findNueCCpi0(std::string const& filename, size_t nevents, const char* recolabel = "pandoraTrack")
{
  ...
  for (gallery::Event ev(filenames); !ev.atEnd(); ev.next()) {
      ...
      if ( abs(neutrino.Lepton().PdgCode()) != 11 || // not nu_e
	   ...
	   neutrino.InteractionType() != simb::kResCCNuNeutronPi0 ) // not CC with n and pi0 in final state
	  continue;
