...
void
findNNbar(std::string const& filename, size_t nevents, const char* recolabel = "pandoraTrack")
{
  ...
  for (gallery::Event ev(filenames); !ev.atEnd(); ev.next()) {
      ...
      // find vertex
      auto aparticle = genparts.GetParticle(0);
