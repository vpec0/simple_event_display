...
void
findGoodAtmonu(std::string const& filename, size_t nevents, const char* recolabel = "pandoraTrack")
{
  ...
  for (gallery::Event ev(filenames); !ev.atEnd(); ev.next()) {
	  ...
	  if (abs(ipart->PdgCode()) == 13) break;

      auto muon = ipart;

      if (muon == partstop || muon -> Mother() != 0 ) continue; // no muon in initial state

      double mulen = muon->Trajectory().TotalLength() ;
      if ( mulen > 100. || mulen < 30.) continue; // too long/short muon


      // find the proton
      for ( ;ipart != partstop; ++ipart) {
	  if ( ipart->PdgCode() == 2212 )
	      break;
      }
      auto proton = ipart;
      if (proton == partstop || proton->Mother() != 0)
	  continue; // not primary proton

      double plen = proton->Trajectory().TotalLength() ;
      if ( plen < 5.) continue; // too short proton
